<?php 
	...
	if ( $c_n == 0 ) { // only leaf ?>
		...
		<div>
    		<a href="./blog/category<?php echo $node->element->{"path"};?>" class="<?php echo $highlight; ?>"><?php echo $node->element->{'title'};?></a>
		</div>
				...
				<div class="panel-body">
					<ul>
                        <?php 
                        	...
                        	for( $i = 0; $i < $c_n; $i++ ){
                        		\Dsc\Request::internal( '\Blog\Site\Controllers\Post->displayCategoryNode', array( $node->nodes[$keys[$i]], $selected_categories ) );
                        	}
                        ?>
					</ul>
				</div>
